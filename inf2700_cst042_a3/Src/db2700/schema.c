...
tbl_p nested_loop_join(tbl_p r, tbl_p s)
{
  ...
  schema_p sch1 = r->sch;
  schema_p sch2 = s->sch;
  if(sch2 == NULL) return NULL;
  field_desc_p f1;
  field_desc_p f2;
  schema_p res_sch;
  tbl_p ret;
  char tmp_name[30] = "tmp_schema";

  for(f1 = sch1->first; f1 != NULL; f1 = f1->next)
  {
    for(f2 = sch2->first; f2 != NULL; f2 = f2->next)
    {
      /*test to chech if names are qual, and type is int*/
     if(strcmp (f1->name, f2->name) == 0)
      {
        if ( f1->type != INT_TYPE || f2->type != INT_TYPE )
        {
          put_msg (ERROR, "f1: \"%s\" or f2: \"%s\" is not an integer field.\n", f1->type, f2->type);
          return NULL;
        }

        res_sch = copy_new_schema (sch1, sch2, tmp_name);
        ret = loop_join(sch1, sch2, res_sch, f1->offset, f2->offset);
        //ret = block_loop_join(sch1, sch2, res_sch, f1->offset, f2->offset);
      }
    }
  }
...
tbl_p loop_join(schema_p s1, schema_p s2, schema_p dest, int offset1, int offset2)
{
  pager_profiler_reset();
  int pos1, pos2, rec_val1, rec_val2;
  record rec1 = new_record(s1);
  record rec2 = new_record(s2);
  record rec_des = new_record(dest);
  page_p pg2;

  set_tbl_position(s1->tbl, TBL_BEG);
  set_tbl_position(s2->tbl, TBL_BEG);

  page_p pg1 = get_page_for_next_record(s1);

  if (pg1 == NULL) return 0;

  /*Loop through the first schemas pages record to find the int-value that
   *is the same as the schema 2's int-value.*/
  for ( ; pg1 != NULL; pg1 = get_page_for_next_record(s1))
  {
    /*Get and set the pages position. Use this with offset
     *to calculate where the record value is.
     *This record value is compared with the other schema/pages record value */
    pos1 = page_current_pos(pg1);
    page_set_current_pos(pg1, pos1);
    get_page_record(pg1, rec1, s1);
    rec_val1 = page_get_int_at (pg1, (pos1 + offset1));

    /*To start at the beginning for every iteration*/
    set_tbl_position(s2->tbl, TBL_BEG);

    /*Find pos, record for schema 2*/
    for (pg2 = get_page_for_next_record(s2); pg2 != NULL; pg2 = get_page_for_next_record(s2))
    {
      /*Get and set the pages position. Use this with offset
       *to calculate where the record value is.*/
      pos2 = page_current_pos(pg2);
      page_set_current_pos(pg2, pos2);
      get_page_record(pg2, rec2, s2);
      rec_val2 = page_get_int_at (pg2, (pos2 + offset2));

      /*If the int are equal, they get filled in new record
       *If is overwritten if they are the same.
       *Then append all this to new record and schema
       *Set current position for not overwriting in next iteration
       */
      if (rec_val1 == rec_val2)
      {
        fill_total_sub_record(rec_des, dest, rec1, s1, rec2, s2);
        append_record(rec_des, dest);
        page_set_current_pos(pg2, pos2 + s2->len);
      }
      else
      {
        /*Update current position for page 2*/
        page_set_current_pos(pg2, pos2 + s2->len);
      }
    }
    /*Update current position for page 1*/
    page_set_current_pos(pg1, pos1 + s1->len);
  }
  release_record(rec_des, dest);
  put_pager_profiler_info(INFO);
  return dest->tbl;
}

// tbl_p block_loop_join(schema_p s1, schema_p s2, schema_p dest, int offset1, int offset2)
//  {
//   int pos1, pos2, rec_val1, rec_val2;
//   record rec1 = new_record(s1);
//   record rec2 = new_record(s2);
...
//   page_p pg1 = get_page_for_next_record(s1);
//   if (pg1 == NULL) return 0;
//   page_p pg2 = get_page_for_next_record(s2);

//   int rec_pr_blk1 = (BLOCK_SIZE - PAGE_HEADER_SIZE)/ s1->len;
//   int blk_nr1 = page_block_nr(pg1);

//   int rec_pr_blk2 = (BLOCK_SIZE - PAGE_HEADER_SIZE)/ s2->len;
//   int blk_nr2 = page_block_nr(pg2);
...
//     /*Find pos, record for schema 2*/
//     for ( ; pg2 != NULL; pg2 = get_page_for_next_record(s2))
//     {
//       /*Get and set the pages position. Use this with offset
//       *to calculate where the record value is.*/
//       pos2 = page_current_pos(pg2);
...
//       if(get_record(rec1) < rec_pr_blk1)
//       {
...
/*Copy schema 1 and 2 to new destination schema without duplicates*/
schema_p copy_new_schema (schema_p s1, schema_p s2, const char *dest_name)
{
  if (s1 == NULL ) return NULL;
  if (s2 == NULL ) return NULL;

  schema_p dest = new_schema (dest_name);
  field_desc_p f1 = s1->first;
  field_desc_p f2 = s2->first;
  /*put every field in s1 in new schema*/
  for ( ; f1 != NULL; f1 = f1->next)
  {
    add_field (dest, copy_field (f1));
  }

  /*put every field in s2 if the names isn't the same*/
  for ( ; f2 != NULL; f2 = f2->next)
  {
    if (get_field(dest, f2->name) == NULL)
    {
      add_field (dest, copy_field (f2));
    }
  }
  return dest;
}

/*Fill sub records with very table values/names*/
void fill_total_sub_record ( record dest_r, schema_p dest_s, record src_r1, schema_p src_s1, record src_r2, schema_p src_s2)
{
  field_desc_p src_f1, src_f2, dest_f;
  int i = 0, j = 0;

  /*Go through schema 1 and put record in destination record*/
  for (dest_f = dest_s->first; i < src_s1->num_fields ; dest_f = dest_f->next, i++ )
  {
    for (j = 0, src_f1 = src_s1->first; strcmp(src_f1->name, dest_f->name) != 0; j++, src_f1 = src_f1->next);

    if (is_int_field (dest_f))
      assign_int_field (dest_r[i], *(int *)src_r1[j]);
    else
      assign_str_field (dest_r[i], (char *)src_r1[j]);
  }

  /*Go through schema 2 and put this record also in destination record. Start where schema 1 ends */
  for ( ; dest_f != NULL; dest_f = dest_f->next, i++ )
  {
