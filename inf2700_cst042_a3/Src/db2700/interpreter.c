...
{
  char in_str[200] = "", from_str[100] = "", tbl2[100] = "";
  char *where_str, *p, *join_str;
  char attr[11] = "", op[3] = "";
  int val;
  tbl_p where_tbl = NULL, res_tbl = NULL, from_tbl = NULL, jointbl1 = NULL, jointbl2 = NULL;
  char *attrs[10];
  ...
  *p = '\0';
  ...
  /*JOIIIIIN*/
  join_str = strstr(p, " join ");
  if(join_str != NULL) join_str += 6;
  
  if (sscanf(p, "%s %*s %s", from_str, tbl2) == 2)
  {
    jointbl1 = get_table(from_str);

    if(jointbl1 == NULL) {
      put_msg(ERROR, "What table\n");
      return;
    }
    jointbl2 = get_table(tbl2);
    if(jointbl2 == NULL)
    {
      put_msg(ERROR, "What table\n");
      return;
    ...
    from_tbl = nested_loop_join(jointbl1, jointbl2);
    from_tbl = get_table("tmp_schema");
  } 
  else
  {
