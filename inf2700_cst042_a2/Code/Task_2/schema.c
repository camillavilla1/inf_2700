...
static int int_bigger(int x, int y)
{
  return x > y;
}

static int int_bigger_equal(int x, int y)
{
  return x >= y;
}
...
fptr find_func(char *op)
{
  /*Find func that the op receive from typer/writer/user...*/
  if (strcmp(op, "=="))
  {
    return int_equal;
  }
...
tbl_p table_search (tbl_p t, char *attr, char *op, int val)
{
  ...
  set_tbl_position ( t, TBL_BEG );
  while ( find_record_int_val (rec, s, f->offset, find_func(op), val) )
    {
